...
import { NotificationItem, Task, UserProfile } from "../types";
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 w-full border-b border-zinc-200/80 dark:border-zinc-800/80 bg-zinc-100/95 dark:bg-zinc-900/95 backdrop-blur-md">
      <div className="flex h-16 items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {!isSidebarOpen && (
            <div className="hidden lg:flex items-center gap-2 pr-2 border-r border-zinc-300 dark:border-zinc-700">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-300 dark:bg-indigo-900 text-indigo-950 dark:text-indigo-200 shadow-xs">
          ...
          <div className="flex items-center gap-2.5">
            <div className="hidden sm:flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-400">
              <CurrentIcon className="h-4 w-4" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-display text-sm sm:text-base font-extrabold text-zinc-900 dark:text-zinc-100 leading-tight">
                  {tabTitles[activeTab].title}
                </span>
                <span className="hidden sm:inline-flex items-center rounded-md bg-indigo-100 dark:bg-indigo-950/40 px-1.5 py-0.5 text-[10px] font-bold text-indigo-700 dark:text-indigo-400">
                  Active
                </span>
              </div>
              <p className="hidden md:block text-[11px] text-zinc-600 dark:text-zinc-400">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              ...
              {urgentTasks.length > 0 && (
                <span className={`ml-1 rounded-full px-1.5 py-0.2 text-[10px] font-black ${
                  activeTab === "tasks" ? "bg-indigo-400 text-indigo-950" : "bg-amber-400 text-amber-950"
                }`}>
                  {urgentTasks.length}
                </span>
              )}
          ...
          {pushStatus !== "granted" ? (
            ...
            <div
              ...
              className="hidden items-center gap-1 rounded-xl bg-emerald-100 dark:bg-emerald-950/40 px-2.5 py-1.5 text-[11px] font-bold text-emerald-800 dark:text-emerald-400 ring-1 ring-inset ring-emerald-600/30 md:flex"
            >
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-black text-zinc-950 shadow-xs">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-3xl border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 p-4 shadow-2xl z-50 animate-in fade-in zoom-in-95">
                <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-2.5">
                  <div className="flex items-center gap-2">
                    <Bell className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                    <span className="text-xs sm:text-sm font-bold font-display text-zinc-900 dark:text-zinc-100">
                ...
                <div className="mt-3 max-h-80 space-y-2 overflow-y-auto">
                  {urgentTasks.length > 0 && (
                    <div className="rounded-2xl border border-amber-300 dark:border-amber-900/50 bg-amber-100 dark:bg-amber-950/40 p-3 text-xs text-amber-950 dark:text-amber-200">
                      ...
                      <div className="mt-1.5 space-y-1">
                        {urgentTasks.slice(0, 2).map((ut) => (
                          <div key={ut.id} className="flex items-center justify-between font-medium">
                            ...
                            <span className="ml-2 font-mono text-[10px] text-amber-800 dark:text-amber-400">
                              Due {ut.dueDate}
                            </span>
                          </div>
                        ))}
                  ...
                  {notifications.length === 0 ? (
                    <div className="py-8 text-center text-xs text-zinc-500 dark:text-zinc-400">
                      <CheckCircle2 className="mx-auto mb-1 h-6 w-6 text-zinc-400 dark:text-zinc-600" />
                    ...
                    notifications.map((n) => (
                      <div
                        ...
                        className={`group relative rounded-2xl border p-3 transition-colors ${
                          n.read
                            ? "border-zinc-200 dark:border-zinc-800/80 bg-zinc-200/50 dark:bg-zinc-850/50 text-zinc-700 dark:text-zinc-400"
                            : "border-indigo-200 dark:border-indigo-900/50 bg-indigo-100/60 dark:bg-indigo-950/30 text-zinc-900 dark:text-zinc-200 font-medium"
                        }`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2">
                            {n.type === "due-soon" ? (
                              <Clock className="mt-0.5 h-3.5 w-3.5 text-rose-600 shrink-0" />
                            ) : n.type === "study-group" ? (
                              <Users className="mt-0.5 h-3.5 w-3.5 text-purple-600 shrink-0" />
                            ) : (
                              <Sparkles className="mt-0.5 h-3.5 w-3.5 text-amber-600 shrink-0" />
                            )}
                            <div>
                              <div className="text-xs font-bold text-zinc-900 dark:text-zinc-100">
                                {n.title}
                              </div>
                              <p className="mt-0.5 text-xs text-zinc-700 dark:text-zinc-300 leading-relaxed">
                                {n.message}
                              </p>
                ...
                <div className="mt-2.5 border-t border-zinc-200 dark:border-zinc-800 pt-2 text-center">
                  <button
                    onClick={() => {
          ...
          {/* User Profile Avatar */}
          <div className="flex items-center gap-2 pl-1 sm:pl-2 border-l border-zinc-300 dark:border-zinc-700">
            {user.photoURL ? (
              <img
                src={user.photoURL}
              ...
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-300 dark:bg-indigo-900 text-indigo-950 dark:text-indigo-200 font-black text-xs shadow-xs">
